...
export function applyTextInput(cm, inserted, deleted, sel, origin) {
    ...
    for (let i = sel.ranges.length - 1; i >= 0; i--) {
        ...
        let changeEvent = { from: from, to: to, text: multiPaste ? multiPaste[i % multiPaste.length] : textLines, origin: origin || (paste ? "paste" : cm.state.cutIncoming > recent ? "cut" : "+input") };
