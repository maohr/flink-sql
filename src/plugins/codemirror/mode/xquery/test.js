...
(function() {
    ...
    MT("testEmptySequenceKeyword", '[string "foo"] [keyword instance] [keyword of] [keyword empty-sequence]()');

    MT("testMultiAttr", '[tag <p ][attribute a1]=[string "foo"] [attribute a2]=[string "bar"][tag >][variable hello] [variable world][tag </p>]');
    ...
    MT("test EQName variable", '[keyword declare] [keyword variable] [variable $"http://www.example.com/ns/my":var] [keyword :=] [atom 12][variable ;]', '[tag <out>]{[variable $"http://www.example.com/ns/my":var]}[tag </out>]');
    ...
    MT("testProcessingInstructions", "[def&variable data]([comment&meta <?target content?>]) [keyword instance] [keyword of] [atom xs:string]");
