...
(function() {
    ...
    MT("atMediaMaxWidthNested", "[def @media] [attribute screen] [keyword and] ([property max-width]: [number 25px]) { [tag foo] { } }");
    ...
    MT("atMediaUppercase", "[def @MEDIA] ([property orienTAtion]: [keyword landScape]) { }");
    ...
    MT("tagColorKeyword", "[tag foo] {", "  [property color]: [keyword black];", "  [property color]: [keyword navy];", "  [property color]: [keyword yellow];", "}");
    ...
    MT("tagTwoPropertiesURL", "[tag foo] { [property background]: [variable&callee url]([string //example.com/foo.png]); [property padding]: [number 0]; }");

    MT("indent_tagSelector", "[tag strong], [tag em] {", "  [property background]: [variable&callee rgba](", "    [number 255], [number 255], [number 0], [number .2]", "  );", "}");

    MT("indent_atMedia", "[def @media] {", "  [tag foo] {", "    [property color]:", "      [keyword yellow];", "  }", "}");

    MT("indent_comma", "[tag foo] {", "  [property font-family]: [variable verdana],", "    [atom sans-serif];", "}");

    MT("indent_parentheses", "[tag foo]:[variable-3 before] {", "  [property background]: [variable&callee url](", "[string     blahblah]", "[string     etc]", "[string   ]) [keyword !important];", "}");
    ...
    MT("css_variable", ":[variable-3 root] {", "  [variable-2 --main-color]: [atom #06c];", "}", "[tag h1][builtin #foo] {", "  [property color]: [variable&callee var]([variable-2 --main-color]);", "}");

    MT("blank_css_variable", ":[variable-3 root] {", "  [variable-2 --]: [atom #06c];", "}", "[tag h1][builtin #foo] {", "  [property color]: [variable&callee var]([variable-2 --]);", "}");
    ...
    MT("document_url", "[def @document] [variable&callee url]([string http://blah]) { [qualifier .class] { } }");

    MT("document_urlPrefix", "[def @document] [variable&callee url-prefix]([string https://]) { [builtin #id] { } }");

    MT("document_domain", "[def @document] [variable&callee domain]([string blah.com]) { [tag foo] { } }");

    MT("document_regexp", '[def @document] [variable&callee regexp]([string ".*blah.+"]) { [builtin #id] { } }');
    ...
    MT("counter-style-symbols", '[tag ol] { [property list-style]: [variable&callee symbols]([atom cyclic] [string "*"] [string "\\2020"] [string "\\2021"] [string "\\A7"]); }');

    MT("comment-does-not-disrupt", "[def @font-face] [comment /* foo */] {", "  [property src]: [variable&callee url]([string x]);", "  [property font-family]: [variable One];", "}");
})();
