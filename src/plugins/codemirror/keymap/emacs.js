...
(function(mod) {
    ...
    function kill(cm, from, to, ring, text) {
        ...
        if (ring == "grow" && lastKill && lastKill.cm == cm && posEq(from, lastKill.pos) && cm.isClean(lastKill.gen)) growRingTop(text);
    ...
    function byExpr(cm, pos, dir) {
        var wrap;
        if (cm.findMatchingBracket && (wrap = cm.findMatchingBracket(pos, { strict: true })) && wrap.match && (wrap.forward ? 1 : -1) == dir) return dir > 0 ? Pos(wrap.to.line, wrap.to.ch + 1) : wrap.to;
    ...
    function getInput(cm, msg, f) {
        if (cm.openDialog) cm.openDialog(msg + ': <input type="text" style="width: 10em"/>', f, { bottom: true });
        else f(prompt(msg, ""));
    }
    ...
    function toEnclosingExpr(cm) {
        ...
        while (line >= cm.firstLine()) {
            ...
            for (var i = ch == null ? text.length : ch; i > 0; ) {
                ...
                else if (/[\(\{\[]/.test(ch) && (!stack.length || stack.pop() != ch)) return cm.extendSelection(Pos(line, i));
            }
            --line;
            ch = null;
        }
