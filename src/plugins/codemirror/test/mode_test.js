...
(function() {
    ...
    function compare(text, expected, mode) {
        ...
        if (observedOutput.indentFailures) {
            for (var i = 0; i < observedOutput.indentFailures.length; i++) s += "<div class='mt-test mt-fail'>" + esc(observedOutput.indentFailures[i]) + "</div>";
        }
    ...
    function highlight(string, mode) {
        ...
        for (var i = 0; i < lines.length; ++i) {
            ...
            if (mode.indent) {
                ...
                if (indent != CodeMirror.Pass && indent != ws.length) (st.indentFailures || (st.indentFailures = [])).push("Indentation of line " + (i + 1) + " is " + indent + " (expected " + ws.length + ")");
            }
            ...
            while (!stream.eol()) {
                for (var j = 0; j < 10 && stream.start >= stream.pos; j++) var compare = mode.token(stream, state);
                if (j == 10) throw new Failure("Failed to advance the stream." + stream.string + " " + stream.pos);
    ...
    function highlightOutputsDifferent(o1, o2) {
        ...
        for (var i = 0; i < minLen; ++i) if (o1[i].style != o2[i].style || o1[i].text != o2[i].text) return i;
