...
function byClassName(elt, cls) {
    ...
    function search(elt) {
        ...
        for (var i = 0, e = elt.childNodes.length; i < e; ++i) search(elt.childNodes[i]);
    }
    search(elt);
    return found;
}
...
testCM("posFromIndex", function(cm) {
    cm.setValue("This function should\n" + "convert a zero based index\n" + "to line and ch.");
    ...
    for (var i = 0; i < examples.length; i++) {
        ...
        if (example.index >= 0 && example.index < 64) eq(cm.indexFromPos(pos), example.index);
    }
});
...
testCM("markTextMultiLine", function(cm) {
    ...
    forEach(
        ...
        function(test) {
            ...
            var r = cm.markText(Pos(0, 5), Pos(2, 5), { className: "CodeMirror-matchingbracket" });
...
testCM(
    ...
    function(cm) {
        ...
        marker1 = cm.markText(Pos(0, 1), Pos(0, 3), { className: "CodeMirror-matchingbracket" });
        marker2 = cm.markText(Pos(0, 0), Pos(2, 1), { className: "CodeMirror-matchingbracket" });
...
testCM(
    ...
    function(cm) {
        ...
        is(!m1.find());
        var m2 = cm.markText(Pos(0, 1), Pos(0, 2), { clearWhenEmpty: false, inclusiveLeft: true, inclusiveRight: true });
...
testCM(
    ...
    function(cm) {
        function present() {
            ...
            for (var i = 0; i < spans.length; i++) if (spans[i].style.color && spans[i].textContent == "cdef") return true;
        }
...
testCM(
    ...
    function(cm) {
        function present() {
            var spans = cm.display.lineDiv.getElementsByTagName("span");
            for (var i = 0; i < spans.length; i++) if (spans[i].getAttribute("label") == "label" && spans[i].textContent == "cdef") return true;
        }
...
testCM("randomCollapsedRanges", function(cm) {
    ...
    cm.operation(function() {
        for (var i = 0; i < 200; i++) {
            ...
            else cm.markText(start, Pos(start.line, start.ch + 4), { className: "foo" });
        }
    });
});
...
testCM(
    ...
    function(cm) {
        ...
        try {
            cm.markText(Pos(0, 1), Pos(0, 2), { collapsed: true, inclusiveLeft: true });
        } catch (e) {
            var caught = e;
        }
        ...
        try {
            cm.markText(Pos(2, 0), Pos(2, 1), { collapsed: true, inclusiveRight: true });
        } catch (e) {
            var caught = e;
        }
        is(caught && /overlap/i.test(caught.message));
...
testCM(
    ...
    function(cm) {
        ...
        cm.replaceRange("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n!\n", Pos(2, 0));
        forEach([Pos(0, doc.length), Pos(0, doc.length - 1), Pos(0, 0), Pos(1, doc.length), Pos(1, doc.length - 1)], function(pos) {
...
testCM(
    ...
    function(cm) {
        ...
        is(cursorBox.bottom < editorBox.top + cm.getScrollerElement().clientHeight, "bottom line visible");
    },
    { lineNumbers: true }
);
...
testCM(
    "groupsAndWhitespace",
    function(cm) {
        var positions = [Pos(0, 0), Pos(0, 2), Pos(0, 5), Pos(0, 9), Pos(0, 11), Pos(1, 0), Pos(1, 2), Pos(1, 5)];
...
testCM(
    ...
    function(cm) {
        if (cm.getOption("inputStyle") != "textarea") return;
        forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج", "خحcd", "1خحcd", "abcdeح1ج", "خمرحبها مها!", "foobarر", "خ ة ق", '<img src="/בדיקה3.jpg">', "يتم السحب في 05 فبراير 2014"], function(line) {
            cm.setValue(line + "\n");
...
testCM(
    ...
    function(cm) {
        function mark(ll, cl, lr, cr, at) {
            return cm.markText(Pos(ll, cl), Pos(lr, cr), { readOnly: true, atomic: at });
        }
        var m = mark(0, 1, 0, 4);
        cm.setCursor(Pos(0, 2));
...
testCM(
    ...
    function(cm) {
        function sendKey(code) {
            cm.triggerOnKeyDown({ type: "keydown", keyCode: code, preventDefault: function() {}, stopPropagation: function() {} });
        }
...
function mouseDown(cm, button, pos, mods) {
    ...
    var event = { type: "mousedown", preventDefault: Math.min, which: button, target: cm.display.lineDiv, clientX: coords.left, clientY: coords.top };
...
testCM(
    ...
    function(cm) {
        cm.on("beforeChange", function(cm, change) {
            var text = [];
            for (var i = 0; i < change.text.length; ++i) text.push(change.text[i].replace(/\s/g, "_"));
...
testCM("beforeSelectionChange", function(cm) {
    ...
    cm.on("beforeSelectionChange", function(cm, obj) {
        obj.update([{ anchor: notAtEnd(cm, obj.ranges[0].anchor), head: notAtEnd(cm, obj.ranges[0].head) }]);
    });
...
var getChar = function(noExtending) {
    ...
    } while ([0x90].indexOf(res.charCodeAt(0)) != -1 || (noExtending && extendingChars.test(res)));
    return res;
};
