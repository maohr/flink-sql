...
var endOfDocument = makeCursor(lines.length - 1, lines[lines.length - 1].length);
...
var oChars = [new Pos(charLine.line, 1), new Pos(charLine.line, 3), new Pos(charLine.line, 7)];
var pChars = [new Pos(charLine.line, 2), new Pos(charLine.line, 4), new Pos(charLine.line, 6), new Pos(charLine.line, 8)];
var numChars = [new Pos(charLine.line, 10), new Pos(charLine.line, 12), new Pos(charLine.line, 14), new Pos(charLine.line, 16), new Pos(charLine.line, 18)];
var parens1 = {
...
testJumplist("jumplist_?", ["?", "<C-o>"], [2, 3], [2, 3], "dialog");
testJumplist("jumplist_skip_deleted_mark<c-o>", ["*", "n", "n", "k", "d", "k", "<C-o>", "<C-o>", "<C-o>"], [0, 2], [0, 2]);
testJumplist("jumplist_skip_deleted_mark<c-i>", ["*", "n", "n", "k", "d", "k", "<C-o>", "<C-i>", "<C-i>"], [1, 0], [0, 2]);
...
testMotion("ge_startOfDocument", ["g", "e"], makeCursor(0, 0), makeCursor(0, 0));
testMotion("ge_end_to_start", ["1000", "g", "e"], makeCursor(0, 0), endOfDocument);
testMotion("gg", ["g", "g"], makeCursor(lines[0].line, lines[0].textStart), makeCursor(3, 1));
testMotion("gg_repeat", ["3", "g", "g"], makeCursor(lines[2].line, lines[2].textStart));
testMotion("G", "G", makeCursor(lines[lines.length - 1].line, lines[lines.length - 1].textStart), makeCursor(3, 1));
testMotion("G_repeat", ["3", "G"], makeCursor(lines[2].line, lines[2].textStart));
...
testMotion("$_repeat", ["2", "$"], makeCursor(1, lines[1].length - 1), makeCursor(0, 3));
...
testMotion("t", ["t", "p"], offsetCursor(pChars[0], 0, -1), makeCursor(charLine.line, 0));
testMotion("t_repeat", ["2", "t", "p"], offsetCursor(pChars[2], 0, -1), pChars[0]);
...
testVim(
    ...
    },
    {
        lineWrapping: true,
        value:
            "This line is intentionally long. It tests movements of gj and gk over wrapped lines. Starts on the end of this line, then makes a step up and back to set the origin for j and k.\nThis line is supposed to be even longer than the previous. I will jump here and make another wiggle with gj and gk, before I jump back to the line above. Both wiggles should not change my cursor's target character but both j/k and gj/gk change each other's reference position."
    }
);
testVim(
    "gj_gk",
...
testVim(
    "/ and gn selects the appropriate word",
    function(cm, vim, helpers) {
        cm.openDialog = helpers.fakeOpenDialog("match");
        helpers.doKeys("/");
        helpers.assertCursorAt(0, 11);
...
testVim(
    "/ and gn with an associated operator",
    function(cm, vim, helpers) {
        cm.openDialog = helpers.fakeOpenDialog("match");
        helpers.doKeys("/");
        helpers.assertCursorAt(0, 11);
...
testSubstituteConfirm("ex_substitute_confirm_emptydoc", "%s/x/b/c", "", "", "", makeCursor(0, 0));
testSubstituteConfirm("ex_substitute_confirm_nomatch", "%s/x/b/c", "ba a\nbab", "ba a\nbab", "", makeCursor(0, 0));
testSubstituteConfirm("ex_substitute_confirm_accept", "%s/a/b/cg", "ba a\nbab", "bb b\nbbb", "yyy", makeCursor(1, 1));
testSubstituteConfirm("ex_substitute_confirm_random_keys", "%s/a/b/cg", "ba a\nbab", "bb b\nbbb", "ysdkywerty", makeCursor(1, 1));
testSubstituteConfirm("ex_substitute_confirm_some", "%s/a/b/cg", "ba a\nbab", "bb a\nbbb", "yny", makeCursor(1, 1));
testSubstituteConfirm("ex_substitute_confirm_all", "%s/a/b/cg", "ba a\nbab", "bb b\nbbb", "a", makeCursor(1, 1));
testSubstituteConfirm("ex_substitute_confirm_accept_then_all", "%s/a/b/cg", "ba a\nbab", "bb b\nbbb", "ya", makeCursor(1, 1));
testSubstituteConfirm("ex_substitute_confirm_quit", "%s/a/b/cg", "ba a\nbab", "bb a\nbab", "yq", makeCursor(0, 3));
testSubstituteConfirm("ex_substitute_confirm_last", "%s/a/b/cg", "ba a\nbab", "bb b\nbab", "yl", makeCursor(0, 3));
testSubstituteConfirm("ex_substitute_confirm_oneline", "1s/a/b/cg", "ba a\nbab", "bb b\nbab", "yl", makeCursor(0, 3));
testSubstituteConfirm("ex_substitute_confirm_range_accept", "1,2s/a/b/cg", "aa\na \na\na", "bb\nb \na\na", "yyy", makeCursor(1, 0));
testSubstituteConfirm("ex_substitute_confirm_range_some", "1,3s/a/b/cg", "aa\na \na\na", "ba\nb \nb\na", "ynyy", makeCursor(2, 0));
testSubstituteConfirm("ex_substitute_confirm_range_all", "1,3s/a/b/cg", "aa\na \na\na", "bb\nb \nb\na", "a", makeCursor(2, 0));
testSubstituteConfirm("ex_substitute_confirm_range_last", "1,3s/a/b/cg", "aa\na \na\na", "bb\nb \na\na", "yyl", makeCursor(1, 0));
